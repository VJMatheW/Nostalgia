...
let is_new_db = !fs.existsSync(path.resolve(process.env.NOSTALGIC_ROOT, "nostalgia.sqlite"));
...
function initDB(is_new_db) {
    if (is_new_db) {
        db.serialize(function() {
            ...
            db.run('create table objects(o_id integer primary key autoincrement, name varchar not null, parent_id integer not null, o_type varchar,created_on DATE default (datetime("now","localtime")), created_by integer, digitized_date varchar, encoded text)');
