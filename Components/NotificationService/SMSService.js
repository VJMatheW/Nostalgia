...
exports.sendSMS = function( mobile_no, text ){
    return new Promise((resolve, reject)=>{
        try{
            HttpService.get(`${process.env.SMS_HOST}/api/mt/SendSMS?user=${process.env.SMS_USER_NAME}&password=${process.env.SMS_USER_PASSWORD}&senderid=${process.env.SMS_SENDER_ID}&channel=Trans&DCS=0&flashsms=0&number=91${mobile_no}&text=${encodeURIComponent(text)}&route=4`
                ,(res)=>{
                    res = JSON.parse(res)
